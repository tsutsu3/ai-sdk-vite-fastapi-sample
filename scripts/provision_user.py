...
def _create_repo(settings: Settings):
    ...
    return factory.authz(), cosmos_provider
...
async def _main() -> None:
    args = _parse_args()
    # Settings loads from OS env and backend/.env or .env if present.
    settings = Settings()
    repo, provider = _create_repo(settings)
    try:
        if args.csv:
