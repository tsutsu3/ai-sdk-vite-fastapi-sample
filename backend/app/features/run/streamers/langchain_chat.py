...
class LangChainChatStreamer(BaseStreamer):
    ...
    def __init__(self, config: AppConfig, chat_caps: ChatCapabilities) -> None:
        ...
        self._memory_responses = [
            "Here is a sample response from the memory provider.",
            "Memory mode is enabled. Configure a real provider for live responses.",
            "This is a placeholder response from the memory provider.",
        ]
    ...
    async def stream_chat(
        ...
        if provider == "memory":
            response_text = random.choice(self._memory_responses)
    ...
    async def generate_title(
        ...
        if provider == "gcp":
            llm = llm.bind(max_output_tokens=40, temperature=0.2)
        else:
