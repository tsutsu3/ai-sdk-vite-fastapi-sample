...
async def test_archive_and_list_conversations():
    ...
    active = await repo.list_conversations(tenant_id, user_id)
    assert active
    ...
    active_after = await repo.list_conversations(tenant_id, user_id)
    archived_after = await repo.list_archived_conversations(tenant_id, user_id)
